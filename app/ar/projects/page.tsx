...
export default function ArabicProjectsCatalogPage() {
  ...
  // جلب ودمج المشاريع بالكامل قبل إظهار الصفحة
  useEffect(() => {
    const fetchSanityProjects = async () => {
      try {
        ...
        // دمج الاثنين معاً دفعة واحدة
        setAllProjects([...formattedSanityProjects, ...projectsDatabase]);
      } catch (error) {
        console.error("Error fetching projects from Sanity:", error);
        setAllProjects(projectsDatabase); // في حال حدث خطأ، اعرض القديم على الأقل
      } finally {
        setLoading(false);
      }
    };
    ...
    <main className="min-h-screen bg-[#070B14] text-white selection:bg-[#D4AF37] selection:text-[#070B14]">
      ...
      {loading && (
        <div className="text-center py-20 text-[#D4AF37] animate-pulse text-sm">
          جاري تحميل سجل المشاريع الهندسية...
        </div>
      )}
